...
input,select,button,textarea{font-family:'Noto Sans Thai',sans-serif}
...
/* score input — remove spinner */
.score-input::-webkit-outer-spin-button,
.score-input::-webkit-inner-spin-button{-webkit-appearance:none;margin:0}
.score-input{-moz-appearance:textfield}

/* animations */
@keyframes pulse-live{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.35;transform:scale(1.8)}}
.pulse-live{animation:pulse-live 1.6s ease-in-out infinite}

@keyframes fadeslide{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}
.fadeslide{animation:fadeslide .28s cubic-bezier(.22,1,.36,1) both}
        ...
        {!user ? (
          <div className="flex flex-col gap-3">
            ...
            <div className="flex items-center gap-3 my-1">
              ...
              <span className="text-xs" style={{color:C.muted}}>หรือเข้าก๊วนที่มีอยู่แล้ว</span>
              <div className="flex-1 h-px" style={{background:"rgba(255,255,255,.06)"}}/>
            </div>
  ...
  return (
    <header className="sticky top-0 z-50"
      ...
      <div className="w-full px-4 lg:px-6 h-14 flex items-center gap-3">
        {/* Brand */}
        {tenant.logo
          ? <img src={tenant.logo} alt="logo" className="w-8 h-8 rounded-lg object-cover flex-shrink-0 ring-1 ring-white/10"/>
          : <ShuttleIcon size={20}/>
        }
        <span className="font-bold text-sm tracking-wide truncate max-w-[150px]" style={{color:C.gold}}>
          {tenant.name || tenant.id}
        </span>

        {/* Separator */}
        <div className="w-px h-4 mx-1 flex-shrink-0" style={{background:"rgba(255,255,255,.1)"}}/>
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative px-6 lg:px-12 py-12 lg:py-16 max-w-screen-2xl mx-auto">
          <div className="flex items-end justify-between gap-6 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[4px] mb-3" style={{color:C.gold}}>TOURNAMENT</p>
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{border:"1px solid rgba(255,255,255,.07)"}}>
      ...
      <div className="flex items-center px-3 lg:px-5 py-3 text-[10px] font-bold uppercase tracking-[1.5px]"
        ...
        {/* Desktop stat headers */}
        <span className="hidden lg:block w-16 text-right">ชนะ/แข่ง</span>
        <span className="hidden lg:block w-16 text-right">ได้/เสีย</span>
        <span className="hidden lg:block w-14 text-right">+/-</span>
        {/* Mobile compact headers */}
        <span className="lg:hidden w-9 text-right">W</span>
        <span className="lg:hidden w-9 text-right">GD</span>
        <span className="lg:hidden w-9 text-right font-extrabold" style={{color:C.gold}}>Pts</span>
      </div>
      ...
      {rows.map((r,i)=>{
        ...
        return (
          <div key={r.team.id}
            className="flex items-center px-3 lg:px-5 py-3 lg:py-4 transition-colors"
            style={{
            ...
            <div className="w-8 flex-shrink-0 flex items-center justify-center">
              {isFirst
                ...
                : <span className="text-sm font-bold" style={{color:C.muted}}>{i+1}</span>
              }
            </div>

            {/* Team: photos + name */}
            <div className="flex-1 flex items-center gap-2 min-w-0">
              {/* All player avatars stacked horizontally */}
              <div className="flex items-center flex-shrink-0"
                style={{marginRight: players.length > 1 ? "4px" : "0"}}>
                {players.map((p, pi) => (
                  <div key={p.id} style={{marginLeft: pi > 0 ? "-8px" : "0", zIndex: players.length - pi, position:"relative"}}>
                    {/* desktop: md, mobile: xs */}
                    <span className="hidden lg:block"><Av athlete={p} size="md"/></span>
                    <span className="lg:hidden"><Av athlete={p} size="xs"/></span>
                  </div>
                ))}
              </div>
              {/* Name */}
              <span className="truncate font-semibold text-xs lg:text-sm"
            ...
            {/* Desktop stats */}
            <span className="hidden lg:block w-16 text-right text-base tabular-nums font-mono">
              <span style={{color:C.green,fontWeight:700}}>{r.wins}</span>
              <span style={{color:C.muted}}>/{r.played}</span>
            </span>
            <span className="hidden lg:block w-16 text-right text-base tabular-nums font-mono" style={{color:C.dim}}>
              {r.pFor}/{r.pAg}
            </span>
            <span className="hidden lg:block w-14 text-right text-base tabular-nums font-bold font-mono"
              style={{color:r.diff>0?C.green:r.diff<0?C.red:C.muted}}>
              {r.diff>0?"+":""}{r.diff}
            </span>

            {/* Mobile compact stats */}
            <span className="lg:hidden w-9 text-right text-base tabular-nums font-bold"
              style={{color:C.green}}>{r.wins}</span>
            <span className="lg:hidden w-9 text-right text-base tabular-nums font-mono"
              style={{color:r.diff>0?C.green:r.diff<0?C.red:C.muted}}>
              {r.diff>0?"+":""}{r.diff}
            </span>
            <span className="lg:hidden w-9 text-right text-base tabular-nums font-extrabold"
              style={{color: isFirst ? C.gold : C.text}}>
              {r.wins * 3}
            </span>
          </div>
...
function HistoryDetail({ tenantId, tournament, onBack }) {
  ...
  const leaderboard = g => {
    ...
    return teams.map(team=>{
      ...
      mp.forEach(m=>{
        const isA=m.teamA?.id===team.id;let aw=0,bw=0;
        (m.sets||[]).forEach(s=>{const sa=parseInt(s.a)||0,sb=parseInt(s.b)||0;
          if(isA){pFor+=sa;pAg+=sb;if(sa>sb)aw++;else bw++;}
          else{pFor+=sb;pAg+=sa;if(sb>sa)aw++;else bw++;}
        });
        if(aw>bw)wins++;
                ...
                <div key={m.id} className="rounded-2xl px-4 py-3"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 flex items-center gap-2 min-w-0 justify-end">
                      {wB && <span className="text-[9px] font-extrabold tracking-widest flex-shrink-0" style={{color:C.gold}}>WIN</span>}
...
function AdminCreateTournament({ tenantId, onDone, inline }) {
  ...
  const buildAndSave = async () => {
    setSaving(true);
    const tourData = {
      name: tname.trim(), config, groupTeams, status:"active", createdAt:Date.now(),
    ...
    for(let g=0;g<config.groups;g++){
      const teams=groupTeams[g]||[];if(teams.length<2)continue;
      const pairs=generateRoundRobin(teams);
      for(let idx=0;idx<pairs.length;idx++){
        const sets=Array.from({length:config.sets},()=>({a:"",b:""}));
        await dbAddDoc(`tenants/${tenantId}/tournaments/${tid}/matches`,{group:g,teamA:pairs[idx][0],teamB:pairs[idx][1],sets,done:false,order:idx});
      }
    }
